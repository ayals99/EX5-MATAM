        ...
        with Playground(curr_dir) as pg:
           ...
           for file in files:
                ...
                if not os.path.isfile(expected_file):
                    print(f"Test Failed: your code created an unexpected file: '{expected_file}' in directory '{curr_dir}'")
                    success_flag = False
                    break
                output = os.path.join(curr_dir, file)
                if not os.path.isfile(output):
                    print(f"Test Failed: your code did not create the following file: '{file}' in directory '{curr_dir}'")
                    success_flag = False
                    break
                if not filecmp.cmp(output, expected_file):
                    print(f"Test Failed: file content does not match for '{expected_file}' and '{output}' in directory '{curr_dir}'")
                    success_flag = False
                    break
           if success_flag:
